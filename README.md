...
Примечания:
- По умолчанию клиент использует встроенный STUN/TURN-сервер (Coturn), поэтому соединение через NAT чаще всего устанавливается без доп. настройки. Для прод-среды для максимальной надёжности укажите свои TURN-серверы. В docker-compose можно переопределить ICE через VITE_ICE_JSON.
...
- frontend/src/config.ts — строит `ICE_SERVERS` из переменной окружения `VITE_ICE_JSON`. Если она пуста/`[]`, используются встроенные STUN/TURN‑сервера (Coturn).
...
### Примечание для Firefox (Ubuntu)
- В Firefox при наличии 5 и более STUN/TURN URL выводится предупреждение: "WebRTC: Using five or more STUN/TURN servers slows down discovery" и ICE-сбор может замедляться.
- В этом приложении клиент автоматически нормализует ICE-конфиг: удаляет дубликаты URL, отдаёт приоритет TURN и для Firefox ограничивает список до 4 URL (сначала TURN, затем STUN). При политике ICE `relay` STUN-URL отбрасываются.
- Рекомендация: не указывайте чрезмерно длинные списки ICE. Обычно достаточно 1 STUN и 2–3 TURN (udp/tcp/tls) на одном хосте.

### Примечание для Android / Huawei (MatePad и др.)
- На некоторых устройствах Android (особенно Huawei / HarmonyOS / Huawei Browser / WebView‑варианты) встречаются проблемы с кодеками VP9/VP8 (черный экран, зависания, не устанавливается медиасоединение).
- Клиент теперь предпочитает H.264 не только для Safari/iOS, но и для Android/Huawei. Это повышает совместимость и задействует аппаратное ускорение на большинстве устройств.
- При необходимости можно принудительно включить H.264 на всех платформах — установите переменную сборки фронтенда `VITE_FORCE_H264=true`.
- Если на одном из клиентов H.264 отсутствует (редко, в сборках без H.264), соединение может не установиться. В таком случае отключите форсирование (`VITE_FORCE_H264=false`, значение по умолчанию) или используйте совместимый браузер.
- Остальные рекомендации сохраняются: держите список ICE‑серверов компактным, укажите TURN для сложных сетей, проверьте getUserMedia‑разрешения.

v-001

## 5) Админ панель
- URL: `/admin` (во фронтенде). Позволяет:
  - Просматривать активные комнаты и подключения (peerId, время подключения);
  - Отключать (разрывать) конкретное подключение.
- Backend endpoints:
  - GET `/api/admin/connections` — список активных комнат и пиров;
  - DELETE `/api/admin/connections/{token}/{peerId}` — принудительно разорвать подключение.
- Просмотр видео: админ может открыть комнату по ссылке из панели. Важно: архитектура MVP — P2P на 2 участника, поэтому одновременный «просмотр» третьим пользователем невозможен без изменения архитектуры (SFU/MCU). Чтобы увидеть видео, админ может открыть комнату как один из двух участников.
